...
/*   Updated: 2024/08/18 08:56:00 by m0rgenstern      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_exit_status	ft_test_ft_parsing(int *err)
{
	...
	section(__FUNCTION__);
	*err = 0;
	*err = ft_check(ft_parsing(ARGC_ARGV, ok_pos), SUCCESS, T_INT, sizeof(int));
	*err += ft_check(ft_parsing(ARGC_ARGV, ok_neg), SUCCESS, T_INT, sizeof(int));
	*err += ft_check(ft_parsing(ARGC_SPLIT, ok_split), SUCCESS, T_INT, sizeof(int));

	*err += ft_check(ft_parsing(ARGC_ARGV, ko_not_nb), ERR, T_INT, sizeof(int));
	*err += ft_check(ft_parsing(ARGC_ARGV, ko_double), ERR, T_INT, sizeof(int));
	*err += ft_check(ft_parsing(ARGC_SPLIT, ko_split), ERR, T_INT, sizeof(int));
	if (*err)
		return (*err);
	return (SUCCESS);
}
